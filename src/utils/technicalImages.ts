...
export const TECHNICAL_IMAGE_LIBRARY: TechnicalImageSuggestion[] = [
  ...
  {
    id: 'cad-estilete-lamina',
    title: 'Estiletes, Lâminas e Consumíveis Técnicos (Martor / Slice)',
    url: '/assets/components/estilete-lamina.svg',
    category: 'MATERIAL DE USO/CONSUMO',
    tags: ['estilete', 'lamina', 'espatula', 'cortador', 'bisturi', 'trena', 'martor', 'slice', 'cadeado', 'consumivel', 'uso', 'consumo'],
  },
  {
...
export function resolveItemImage(item: { codigo?: string; descricao?: string; categoria?: string }): string {
  ...
  // 34. Estiletes, Lâminas e Consumíveis de Uso
  if (/ESTILETE|LAMINA|ESPATULA|CORTADOR|BISTURI|TESOURA.*SEGURANCA|CADEADO|PREMISSE/i.test(all)) {
    return '/assets/components/estilete-lamina.svg';
  }

  // Fallback por categoria oficial Manutamaki
  if (cat.includes('USO') || cat.includes('CONSUMO')) return '/assets/components/estilete-lamina.svg';
  ...
  if (cat.includes('UNIFORME')) return '/assets/components/uniforme-operacional.svg';
